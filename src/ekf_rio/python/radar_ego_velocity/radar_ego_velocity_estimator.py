...
class RadarEgoVelocityEstimator:
    ...
    def __init__(self):
        ...
        self.setRansacIter()
        rospy.loginfo('ransac iteration is' + self.ransac_iter)
    ...
    def estimate1(self, radar_scan_msg, v_r, sigma_v_r):
        P_v_r = np.eye(3)
        inlier_radar_msg = PointCloud2()
        success = self.estimate2(radar_scan_msg, v_r, P_v_r, inlier_radar_msg)
    ...
    def estimate2(self, radar_scan_msg, v_r, P_v_r, inlier_radar_msg):
        radar_scan_inlier = []
        success = self.estimate4(radar_scan_inlier, v_r, P_v_r, radar_scan_inlier)
    ...
    def estimate4(self, rdar_scan_msg, v_r, P_v_r, inlier_radar, C_stab_r = np.eye(3)):
        ...
        if pcl2msgToPcl(rdar_scan_msg, radar_scan):
            ...
            for i in range(len(radar_scan)):
                ...
                azimuth = np.arctan2(target.x, target.y) - np.pi/2
                ...
                np.abs(elevation) < np.deg2rad(self.config_.elevation_thresh_deg):
                    ...
                    if p_stab[2] > self.config_.filter_min_z and p_stab[2] < self.config_.filter_max_z:
                        v = np.array([azimuth, elevation, target.x, target.y, target.z, target.snr_db, target.x / r, target.z / r, \
        ...
        if len(valid_target) > 2:
            ...
            if median < self.config_.thresh_zero_velocity:
                ...
                for item in valid_target:
                    if np.abs(item[REEIndices.v_d.value]) < self.config_.thresh_zero_velocity:
                ...
                if self.config_.use_ransac:
                    ...
                    for item in valid_target: radar_data_inlier.append(toRadarPointCloudType(item))
    ...
    def solve3DLsqRansac(self, radar_data, v_r, P_v_r):
        ...
        if len(inlier_idx_best) > 0 :
            ...
            for i in len(inlier_idx_best): radar_data_inlier[i][:] = radar_data[inlier_idx_best[i]][:]
    ...
    def solve3DLsq(self, radar_data, v_r, P_v_r, estimate_sigma = True):
        ...
        if np.abs(cond) < 1.0e3:
            if self.config_.use_cholesky_instead_of_bdcsvd:
                c = np.linalg.cholesky(HTH)
                # Solve the system of linear equations
                # We have to solve twice because it's a two-step process with a Cholesky decomposition
                t = np.linalg.solve(c.T, H.T @ y)
                v_r[:] = np.linalg.solve(c, t)
            else:
    ...
    def solve3DOdr(self, radar_data, v_r, P_v_r):
        ...
        data = Data(H, y, wd=1./sigma_y**2, we=1./sigma_x**2)

        odr = ODR(data, linear_model, beta0=v_r_odr)
