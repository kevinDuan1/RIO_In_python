...
class VelocityEstimationEvaluator:
    ...
    def analyze(self):
        ...
        if len(self.list_v_gt) == 0:
            ...
            fig_v.suptitle(self.velocity_type + ' Estimation', fontsize="large")

            xyz = "xyz"
            for k in range(3):
                ax_comp[k].plot(v_body_raw[:, 0], v_body_raw[:, k + 1], label="radar")
                ax_comp[k].grid(True)
                ax_comp[k].set_ylabel("v_" + xyz[k] + " [m/s]")
            ...
            fig_v, (ax_comp) = plt.subplots(3, 1, sharex=True, sharey=True, figsize=(10, 8))
            fig_v.suptitle(self.velocity_type + ' Estimation Comparison', fontsize="large")

            fig_err, (ax_v_err) = plt.subplots(3, 1, sharex=True, sharey=True, figsize=(10, 8))
            fig_err.suptitle(self.velocity_type + ' Estimation Error with Estimated Sigmas', fontsize="large")
            ...
            for k in range(3):
                ...
                ax_v_err[k].set_ylabel("err v_" + xyz[k] + " [m/s]")
    ...
    def print_stats(self, err):
        ...
        print("##############################################")
        print("Analysis of the radar ego velocity estimation")
        print("###############################################")
...
if __name__ == "__main__":
    ...
    topic_v = rospy.get_param('~topic_v', "")
    topic_v_gt = rospy.get_param('~topic_v_gt', "")
    velocity_type = rospy.get_param('~velocity_type', "")

    evaluator = VelocityEstimationEvaluator(topic_v, velocity_type, topic_v_gt)
